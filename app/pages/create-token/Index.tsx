...
import { useAdminFeeWallet, useFeeCalculation } from "@/hooks/useAdminFeeWallet";
import { FEE_CONFIG, generateFeeBreakdown, formatWalletAddress } from "@/services/feeExtraction";
...
function getMarketCap(virtualSol: number, virtualTokens: number) {
  return getTokenPrice(virtualSol, virtualTokens) * 1_000_000_000;
}

// ─── Storage ──────────────────────────────────────────────────────────────────
const STORAGE_KEY = "frostdex_tokens_v2_pump";
...
function shortWallet(w: string, len = 4) {
  if (!w || w.length < len * 2) return w || "—";
  return `${w.slice(0, len)}…${w.slice(-len)}`;
}
...
function TokenCard({
  ...
  const progress = Math.min(
    100,
    ((token.virtualSol - FEE_CONFIG.VIRTUAL_SOL_INITIAL) /
      (FEE_CONFIG.GRADUATION_TARGET_SOL - FEE_CONFIG.VIRTUAL_SOL_INITIAL)) *
    100
  );
  const graduated = token.virtualSol >= FEE_CONFIG.GRADUATION_TARGET_SOL;
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        ...
        el.style.border = "1px solid rgba(56,224,248,0.35)";
        el.style.background = "rgba(56,224,248,0.05)";
      }}
      onMouseLeave={(e) => {
        const el = e.currentTarget as HTMLDivElement;
        el.style.border = "1px solid rgba(56,224,248,0.1)";
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        {token.image ? (
          ...
          <div
            style={{
              ...
              height: 52,
              borderRadius: 10,
              background: "linear-gradient(135deg,#38e0f8,#0ecb81)",
              flexShrink: 0,
              display: "flex",
              alignItems: "center",
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontWeight: 700, fontSize: 15, color: "#eaecef" }}>
              {token.name}
            </span>
            <span style={{ fontSize: 11, color: "#38e0f8", background: "rgba(56,224,248,0.1)", borderRadius: 4, padding: "1px 6px" }}>
              ${token.symbol}
            </span>
            {graduated && (
              <span style={{ fontSize: 10, color: "#0ecb81", background: "rgba(14,203,129,0.1)", borderRadius: 4, padding: "1px 6px" }}>
                🎓 GRADUATED
              </span>
            )}
          </div>
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 8, flexWrap: "wrap" }}>
            <span style={{ fontSize: 12, color: "rgba(180,190,210,0.7)" }}>
              Price: <b style={{ color: "#eaecef" }}>{price.toFixed(10)} SOL</b>
            </span>
            <span style={{ fontSize: 12, color: "rgba(180,190,210,0.7)" }}>
              MCap: <b style={{ color: "#38e0f8" }}>{mc.toFixed(2)} SOL</b>
            </span>
          ...
          <div style={{ marginTop: 8 }}>
            ...
            >
              <span>{graduated ? "🎓 Graduated" : `Bonding: ${progress.toFixed(1)}%`}</span>
              <span>{(token.virtualSol - FEE_CONFIG.VIRTUAL_SOL_INITIAL).toFixed(2)} / {FEE_CONFIG.GRADUATION_TARGET_SOL - FEE_CONFIG.VIRTUAL_SOL_INITIAL} SOL</span>
            </div>
            <div
              style={{
                height: 4,
            ...
            >
              <div
                style={{
                  height: "100%",
                  width: `${progress}%`,
                  background: graduated ? "#0ecb81" : "linear-gradient(90deg,#38e0f8,#0ecb81)",
                  borderRadius: 9999,
...
function TradeModal({
  ...
  const buyQuote =
    tab === "buy" && amount
      ? getBuyQuote(token.virtualSol, token.virtualTokens, parseFloat(amount) || 0, FEE_CONFIG.PLATFORM_FEE_BPS)
      : null;
  const sellQuote =
    tab === "sell" && amount
      ? getSellQuote(token.virtualSol, token.virtualTokens, parseFloat(amount) || 0, FEE_CONFIG.PLATFORM_FEE_BPS)
      : null;
  ...
  const handleTrade = () => {
    ...
    setTimeout(() => {
      ...
      if (tab === "buy") {
        const q = getBuyQuote(t.virtualSol, t.virtualTokens, val, FEE_CONFIG.PLATFORM_FEE_BPS);
        ...
        if (t.virtualSol >= FEE_CONFIG.GRADUATION_TARGET_SOL) t.graduated = true;
        t.marketCap = getMarketCap(t.virtualSol, t.virtualTokens);
        setResult(`✅ Bought ~${q.tokensOut.toFixed(2)} ${t.symbol}`);
      } else {
        const q = getSellQuote(t.virtualSol, t.virtualTokens, val, FEE_CONFIG.PLATFORM_FEE_BPS);
        ...
        t.marketCap = getMarketCap(t.virtualSol, t.virtualTokens);
        setResult(`✅ Sold ${val} ${t.symbol} → ${q.solOut.toFixed(4)} SOL`);
      }
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: 16, color: "#eaecef" }}>
                {token.name} <span style={{ color: "#38e0f8", fontSize: 13 }}>${token.symbol}</span>
              </div>
              <div style={{ fontSize: 12, color: "rgba(180,190,210,0.5)" }}>
                by {shortWallet(token.creator)}
              </div>
            </div>
          </div>
          <button
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
          {[
            ...
            >
              <div style={{ fontSize: 11, color: "rgba(180,190,210,0.5)", marginBottom: 3 }}>
                {s.label}
              </div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#eaecef" }}>
                {s.value}
              </div>
            </div>
          ))}
          ...
          {(["buy", "sell"] as const).map((t) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                transition: "all 0.15s",
                background: tab === t ? (t === "buy" ? "#0ecb81" : "#f6465d") : "rgba(255,255,255,0.06)",
                color: tab === t ? "#fff" : "rgba(180,190,210,0.6)",
              }}
            >
              {t === "buy" ? "🟢 BUY" : "🔴 SELL"}
            </button>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              ...
              <span style={{ color: "#38e0f8", fontWeight: 600 }}>
                ...
                {adminWallet ? (
                  ...
                  <span style={{ color: "#f6465d", marginLeft: 4 }}>Not connected</span>
                )}
              </span>
            </div>
          </div>
        ...
        <button
          ...
          style={{
            ...
            background: loading
              ...
              : tab === "buy"
                ? "linear-gradient(135deg,#0ecb81,#38e0f8)"
                : "#f6465d",
            color: "#fff",
        ...
        >
          FrostDex Bonding Curve · Powered by ❄️
        </div>
      </div>
    </div>
  );
...
export default function CreateTokenPage() {
  ...
  const { adminWallet, isVerified, connectAdminWallet, loading: walletLoading } =
    useAdminFeeWallet();
  const { calculateTokenCreationFee } = useFeeCalculation();
  ...
  const totalFee = calculateTokenCreationFee(advancedOptions);
  const initialBuyFee = initialBuyEnabled
    ? (parseFloat(initialBuyAmount) || 0) * (FEE_CONFIG.INITIAL_BUY_FEE_BPS / 10000)
    : 0;
  const totalEstimate = totalFee + initialBuyFee;
  ...
  const handleCreate = async () => {
    ...
    // Wallet verification
    if (!adminWallet || !isVerified) {
      setCreateError("Please connect your admin wallet first");
      return;
    }
    ...
    if (symbol.trim().length > 10) return setCreateError("Symbol must be 10 characters or less");
    ...
    try {
      ...
      let virtualSol = FEE_CONFIG.VIRTUAL_SOL_INITIAL;
      let virtualTokens = FEE_CONFIG.VIRTUAL_TOKENS_INITIAL;
      const history: Trade[] = [];

      if (initialBuyEnabled) {
        const buyAmt = parseFloat(initialBuyAmount) || 0;
        if (buyAmt > 0) {
          const q = getBuyQuote(virtualSol, virtualTokens, buyAmt, FEE_CONFIG.INITIAL_BUY_FEE_BPS);
      ...
      const token: TokenData = {
        ...
        virtualSol,
        virtualTokens,
        graduated: virtualSol >= FEE_CONFIG.GRADUATION_TARGET_SOL,
        advancedOptions: Object.entries(advancedOptions)
          .filter(([, v]) => v)
          .map(([k]) => k),
      ...
      setCreateSuccess(
        `🎉 ${name} ($${symbol.toUpperCase()}) created!\nMint: ${mintAddress}\nFees → ${shortWallet(adminWallet, 6)}`
      );
    ...
    >
      ...
      >
        <div
          style={{
            ...
            inset: 0,
            background:
              "linear-gradient(135deg, rgba(56,224,248,0.06) 0%, rgba(14,203,129,0.04) 50%, rgba(11,14,17,0) 100%)",
          }}
        />
        <div
          style={{
            ...
            right: 0,
            height: 2,
            background: "linear-gradient(90deg, transparent 0%, rgba(56,224,248,0.5) 50%, transparent 100%)",
          }}
        />
        <div style={{ position: "relative", zIndex: 1, padding: "28px 24px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
            ...
            >
              FrostDex Bonding Curve
            </h1>
          </div>
          <p style={{ fontSize: 14, color: "rgba(180,190,210,0.7)", margin: "8px 0 0 42px" }}>
            Create your token with Pump Fun mechanics · Wallet-only authentication · Admin fee collection
          </p>
        </div>
      </div>
      ...
      {!isVerified && (
        ...
        >
          ...
          >
            <span style={{ color: "#f6465d", fontWeight: 600, fontSize: 14 }}>
              ⚠️ Connect Admin Wallet
            </span>
            <input
              type="text"
            ...
            <button
              onClick={() => {
                if (!walletConnectInput.trim()) {
                  setWalletError("Wallet address required");
                  return;
                }
                connectAdminWallet(walletConnectInput.trim());
              }}
              disabled={walletLoading || !walletConnectInput.trim()}
              style={{
                padding: "10px 20px",
                borderRadius: 8,
                border: "none",
                ...
                fontWeight: 600,
                cursor: "pointer",
                opacity: walletLoading || !walletConnectInput.trim() ? 0.6 : 1,
              }}
            >
          ...
          {walletError && (
            <div style={{ fontSize: 12, color: "#f6465d", marginTop: 8 }}>
              {walletError}
            </div>
          )}
        </div>
      ...
      {isVerified && adminWallet && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ color: "#0ecb81", fontWeight: 600 }}>✓ Admin Wallet Connected</span>
            <span style={{ fontFamily: "monospace", color: "rgba(14,203,129,0.8)" }}>
              {shortWallet(adminWallet, 6)}
            </span>
            <span style={{ fontSize: 12, color: "rgba(14,203,129,0.6)" }}>
              (All fees collected to this address)
            </span>
          </div>
        </div>
      )}
      ...
      <div
        style={{
          ...
          gap: 8,
          marginBottom: 24,
          borderBottom: "1px solid rgba(56,224,248,0.1)",
          paddingX: 24,
        }}
      >
        {(["create", "browse"] as const).map((t) => (
          <button
            ...
            style={{
              padding: "12px 24px",
              borderBottom: tab === t ? "2px solid #38e0f8" : "none",
              border: "none",
              background: "transparent",
              color: tab === t ? "#38e0f8" : "rgba(180,190,210,0.6)",
          ...
          >
            {t === "create" ? "🎨 Create Token" : "📊 Browse Tokens"}
          </button>
        ))}
      </div>

      <div style={{ paddingX: 24 }}>
        {tab === "create" && (
          <div
            style={{
              maxWidth: 600,
          ...
          >
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <label style={{ ...labelStyle, marginTop: 12 }}>Symbol *</label>
              <input
                type="text"
                value={symbol}
              ...
              {/* Image Upload */}
              <label style={{ ...labelStyle, marginTop: 12 }}>Token Image *</label>
              <div
                onDrop={handleDrop}
                onDragOver={(e) => e.preventDefault()}
                onClick={() => fileInputRef.current?.click()}
                style={{
                  border: "2px dashed rgba(56,224,248,0.3)",
                  borderRadius: 8,
                  padding: 24,
                  textAlign: "center",
                  cursor: "pointer",
                ...
                onMouseEnter={(e) => {
                  (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(56,224,248,0.6)";
                }}
                onMouseLeave={(e) => {
                  (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(56,224,248,0.3)";
                }}
              >
                {imagePreview ? (
                  <div>
                    <img
                      src={imagePreview}
                      alt="Preview"
                      style={{ maxWidth: 120, maxHeight: 120, borderRadius: 8, marginBottom: 12 }}
                    />
                    <div style={{ fontSize: 12, color: "rgba(180,190,210,0.7)" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 14, color: "rgba(180,190,210,0.8)" }}>
                      Drag image here or click to select
                    </div>
                    <div style={{ fontSize: 12, color: "rgba(180,190,210,0.5)", marginTop: 8 }}>
            ...
            {/* Social Links */}
            <div style={{ marginBottom: 24 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: "#eaecef", marginBottom: 12 }}>
                Social Links
              </h3>
              ...
              <label style={{ ...labelStyle, marginTop: 12 }}>Twitter</label>
              <input
                type="text"
                value={twitter}
              ...
              <label style={{ ...labelStyle, marginTop: 12 }}>Telegram</label>
              <input
                type="text"
                value={telegram}
              ...
              <label style={{ ...labelStyle, marginTop: 12 }}>Discord</label>
              <input
                type="url"
                value={discord}
            ...
            >
              ...
              {initialBuyEnabled && (
                <div style={{ marginTop: 12 }}>
                  ...
                  >
                    Fee (20%): {(parseFloat(initialBuyAmount || "0") * (FEE_CONFIG.INITIAL_BUY_FEE_BPS / 10000)).toFixed(4)} SOL
                  </div>
                </div>
              )}
            </div>
            ...
            >
              ...
              {advancedOpen && (
                <div style={{ marginTop: 12 }}>
                  {Object.entries(FEE_CONFIG.ADVANCED_FEES).map(([key, fee]) => (
                    <label
                      key={key}
                      style={{
                        display: "flex",
            ...
            {(totalFee > 0 || initialBuyFee > 0) && (
              ...
              >
                <div style={{ fontSize: 12, fontWeight: 600, color: "#0ecb81", marginBottom: 8 }}>
                  💰 Fee Breakdown
                </div>
                <div
                  style={{
                ...
                >
                  <span>Token Creation Base:</span>
                  <span>{FEE_CONFIG.TOKEN_CREATION_BASE_FEE} SOL</span>
                </div>
                {Object.entries(advancedOptions)
                  ...
                  .map(([key]) => (
                    ...
                    >
                      ...
                      <span>{FEE_CONFIG.ADVANCED_FEES[key as keyof typeof FEE_CONFIG.ADVANCED_FEES]} SOL</span>
                    </div>
                  ))}
                {initialBuyFee > 0 && (
                  <div
                  ...
                  >
                    <span>Initial Buy Fee (20%):</span>
                    <span>{initialBuyFee.toFixed(4)} SOL</span>
                  </div>
                )}
                <div
                  style={{
                    ...
                    fontWeight: 700,
                    color: "#0ecb81",
                    borderTop: "1px solid rgba(14,203,129,0.2)",
                    paddingTop: 8,
                    marginTop: 8,
                  }}
                >
                  <span>Total:</span>
                  <span>{totalEstimate.toFixed(4)} SOL</span>
                </div>
                <div
                  style={{
                    fontSize: 11,
                ...
                >
                  All fees go to: <span style={{ fontFamily: "monospace" }}>{shortWallet(adminWallet || "wallet")}</span>
                </div>
              </div>
            )}
            ...
            <button
              ...
              disabled={creating || !isVerified || !name.trim() || !symbol.trim()}
              style={{
                width: "100%",
                padding: "14px 0",
                borderRadius: 10,
                ...
                fontWeight: 700,
                fontSize: 16,
                cursor: creating || !isVerified ? "not-allowed" : "pointer",
                background: creating || !isVerified ? "rgba(255,255,255,0.07)" : "linear-gradient(135deg, #38e0f8 0%, #0ecb81 100%)",
                color: "#fff",
                opacity: !isVerified ? 0.5 : 1,
                transition: "opacity 0.15s",
              }}
            >
              {creating ? "Creating…" : !isVerified ? "Connect Wallet First" : "🚀 Create Token"}
            </button>
          </div>
        )}
            ...
            {filteredTokens.length > 0 ? (
              ...
              >
                {filteredTokens.map((t) => (
                  <TokenCard
                    key={t.id}
                    token={t}
                    onTrade={(token) => setSelectedToken(token)}
                  />
                ))}
              </div>
            ) : (
              ...
              >
                ...
                <div style={{ marginTop: 12, fontSize: 14 }}>
                  {search ? "No tokens found" : "No tokens created yet"}
                </div>
              </div>
            )}
          </div>
